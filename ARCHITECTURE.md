...
这是新项目的**设计契约**，不是全部已实现功能清单。当前已实现训练 CSV 边界、BaoStock 单源接口/原始采集、按日历可用时间重放的数据层、一个 20 日动量信号与五日目标的诊断闭环；Agent、可靠性、状态识别、组合优化与交易回测仍未实现。未来模块只有进入相应阶段才创建，不沿用旧选股项目代码。此设计依据本项目的研究目标重新确定。
...
| `adaptive_mas.agents` | 专家预测器，只返回信号，不管理持仓 | P5 量化专家：技术/趋势、横截面因子、流动性；P9 基本面/公告；P10 新闻/宏观。各自是否保留由增量实验决定 |
| `adaptive_mas.reliability` | 概率校准、专家误差/可靠性估计及门控权重 | P6；按标签成熟时间延迟更新。状态条件扩展在 P7 |
...
| `adaptive_mas.backtest`、`adaptive_mas.evaluation` | 驱动历史决策流程并评价成交后的表现 | P4 起建立基线；P8 起验证完整系统 |
...
计划中的每条 `AgentSignal` 至少标明：股票、决策时刻、统一的五交易日预测期限、预期收益、估计不确定性、输入数据截止时间、证据引用和 Agent 版本。确切 Python 类型与序列化形式到 P5 才确定。校准层负责定义可比较的置信度/概率；不能把不同 Agent 自报的数字直接解释为同一尺度。
