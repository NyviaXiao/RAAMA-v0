...
这是新项目的**设计契约**，并标明当前实现边界。P3–P8 首个数值研究 MVP 已贯通点时市场数据、三个量价专家、Ridge 基线、等权/静态/自适应/状态条件融合、Top-K 与逆波动率组合。执行层保留连续持仓 v1，并新增与五日目标匹配的固定期限 v2：决策与成熟结算分账、预登记退出、停牌延期持有，以及现金和现有持仓约束买单。两种协议共用同一成本与流动性假设。其账本按名义金额估值，不是逐股股数/券商成交回放，也不是完整协方差或 CVaR 优化。P9 以后的文本 LLM、基本面/新闻专家、辩论、记忆和强化学习没有实现。代码与真实数据实验见 [MVP_RESEARCH.md](MVP_RESEARCH.md) 和 [M1_FIXED_HORIZON_RESEARCH.md](M1_FIXED_HORIZON_RESEARCH.md)；早期信号诊断见 [AGENT_RESEARCH.md](AGENT_RESEARCH.md)。未实现模块只保留设计，不创建空 API，也不沿用旧选股项目代码。
...
| `backtest` / `evaluation` | 统一策略的目标权重、次日开盘名义成交、停牌/流动性限制、费用、持仓和净值评价；支持连续持仓与五日计划退出两种显式协议，预测标签独立成熟结算 | 将名义金额账本称作逐股成交回放；把不同执行协议的净值当作同一个策略 | P4 连续持仓已实现；路线图 M1 固定期限与双账本首版已实现 |
